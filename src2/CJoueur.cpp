...
void CJoueur::attaque ( pair<int,int> tir, CCoups& e ) {
	...
	// on passe le tir en 2 coordonnées (first, second) et on les envoie par réseau é l'adversaire
	// dans une seule chaéne de caractéres (les 2 coord. sont séparées par un espace)
	ostringstream strout;
	strout << tir.first << " " << tir.second; // l'espace sépare les 2 coordonnées
	*m_pSocCli << strout.str();			   

    // on récupére le résultat de l'attaque, c'est-é-dire la REPONSE de l'adversaire : "dansLEau" ou "touche"
	*m_pSocCli >> str;
	...
	// mise é jour de la structure de données qui comptabilise les tirs (dansLEau ou touche ou coule) du joueur
    // dansLEau
...
void CJoueur::attaqueAdverse ( CArmada* f, CCoups* e ) {
	...
	// dans une seule chaéne de caractéres décodée (2 coord. séparées par un espace)
	*m_pSocCli >> str;
	...
	cout << "RESULTAT du tir adverse... ";
 	...
 	// est-on touché ?
   	while ( (i<num) && (!stop) ) {
		...
		theB = f->getBateau(i);
		// tirAdverse (...) ne renvoie vrai que si la case est touchée pour la premiére fois !!
		if ( theB->tirAdverse (coup) ) {
			...
			if ( theB->estCoule() ) {
				...
				// REPONSE renvoyée é l'adversaire du type "nomDuBateau coule"
				*m_pSocCli << str;
				stop = true;
			}
				...
				// REPONSE renvoyée é l'adversaire du type "nomDuBateau touche"
				*m_pSocCli << str;
				stop = true;
			}
		}
	...
	if ( !stop ) {
    	...
    	// REPONSE renvoyée é l'adversaire : "dansLEau"
    	*m_pSocCli << "dansLEau";
