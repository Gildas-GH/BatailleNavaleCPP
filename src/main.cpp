...
// Simple signature de la fonction dialogueChoix é déclarer ici
int dialogueChoix ();
...
int main() {
	...
	switch (choix) {
		...
		case 0: {
			...
			// Construction de l'interface utilisateur cété serveur
			pGuiServ = new CGui();
			...
			// Le serveur se met é l'écoute d'une connexion client sur ce port
			// Si cela se passe mal, une exception logic_error est lancée (et pas capturée...)
			jouServ.openSocket( PORTCOM );

			// Attendre le client
			// Le message "... en attente du client" doit apparaétre
			jouServ.attenteClient();

			// Récupérer les données du jeu é partir du fichier "flotille.txt" qui se trouve dans /ws.
			...
			// Mise é jour de la GUI du serveur.
			...
			while ( okPlcmt ) {
				...
				// ET récupération de sa réponse (coulé, touché ou plouf) pour mettre é jour pCoupsServ
				jouServ.attaque ( coup, *pCoupsServ );
				
				// Afficher les deux grilles pour le serveur
				cout << *pGuiServ;

				// mise é jour de nbToucheAdverse
				...
				// Dés que le client tire, on lui envoie une réponse : coulé, touché ou plouf
				jouServ.attaqueAdverse (pArmServ, pCoupsServ);
				
				// suite é l'attaque, le serveur peut perdre
		...
		case 1: {
			...
			// Construction de l'interface utilisateur cété client
			pGuiCli = new CGui();
			...
			// Mise é jour de la GUI du client.
			...
			while ( okPlcmt ) {
				...
				// Dés que le serveur tire, on lui envoie une réponse : coulé, touché ou plouf
				jouCli.attaqueAdverse (pArmCli, pCoupsCli);

				// Afficher les deux grilles pour le client
				cout << *pGuiCli;

				// suite é l'attaque, le client peut perdre
				...
				// au client é choisir un tir
				pair<int,int> coup = pGuiCli->choisirAttaque ();

				// envoie d'un tir au serveur par le réseau
				// ET récupération de sa réponse (coulé, touché ou plouf) pour mettre é jour pCoupsCli
